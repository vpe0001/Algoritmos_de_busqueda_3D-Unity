...
El descenso gradiente es un algoritmo iterativo que a partir de unos valores iniciales, itera una función modificando esos valores hacía un mínimo de esa función. El algoritmo termina cuando el cambio que se producen en los valores es menor que la tolerancia indicada.
...
\end{center}
Donde $\alpha$ es el peso que le damos a cuanto de cerca queremos que la ruta suavizada esté de la ruta original, x es el punto de la ruta original y y es el nuevo punto suavizado.
...
\section{Theta*}
Theta A* es un algoritmo desarrollado por A. Nash, K. Daniel, S. Koenig y A. Felner \cite{thetaestrella} \cite{thetaestrellaweb} que modifica el A* original para obtener rutas más realistas.
